...
class ArrowKtTests {
    ...
    internal fun `test catchErr series`() = runBlocking {
        ...
        val eitherLeftRun: Either<AppErr, Map<*, *>> =
            list.catchErrWhenRun(CastClassErr) { it as Map<*, *> }
        ...
        val eitherRightNull: Either<AppErr, String> =
            map["key1"].catchErrWhenNull(NullErr)
        ...
        val eitherRightRun: Either<AppErr, MutableList<String>> =
            list.catchErrWhenRun(CastClassErr) { it as MutableList<String> }
    ...
    internal fun `test flattenCatchErr series`() = runBlocking {
        ...
        val eitherLeftNull: Either<AppErr, Map<*, *>> = map["key2"]
        ...
        val eitherLeftTrue: Either<AppErr, Map<*, *>> = map["key1"]
            .catchErrWhenTrue(AssertErr) { it == null }
        ...
        val eitherLeftRun: Either<AppErr, Map<*, *>> = map["key1"]
            .catchErrWhenTrue(AssertErr) { it == null }
            .flatCatchErrWhenNull(NullErr)
            .flatCatchErrWhenTrue(AssertErr) { it.length < 3 }
    ...
    @Test
    internal fun `test validErr and flatValidErr series`() = runBlocking {
        val list = listOf("value1")
        val map = mapOf("key1" to "value1")
